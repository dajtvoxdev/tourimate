# TouriMate Backend Deployment Script
# Version: 1.0
# Date: $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')

Write-Host "===========================================" -ForegroundColor Cyan
Write-Host "TouriMate Backend Deployment Script" -ForegroundColor Cyan
Write-Host "===========================================" -ForegroundColor Cyan
...
if (Test-Path "tourimate-production.tar.gz") {
    $fileSize = (Get-Item "tourimate-production.tar.gz").Length
    ...
    tar -tzf tourimate-production.tar.gz | Select-Object -First 10 | ForEach-Object { Write-Host "   $_" -ForegroundColor Gray }
    if ((tar -tzf tourimate-production.tar.gz | Measure-Object).Count -gt 10) {
        Write-Host "   ... and $(((tar -tzf tourimate-production.tar.gz | Measure-Object).Count - 10)) more files" -ForegroundColor Gray
    }
} else {
...
try {
    ...
    Stop-Website -Name "TouriMate API Production" -ErrorAction Stop
...
if (Test-Path "C:\inetpub\wwwroot\tourimate-production") {
    ...
    try {
        Move-Item "C:\inetpub\wwwroot\tourimate-production" $backupPath -Force
...
try {
    ...
    Write-Host "✅ Directory created: C:\inetpub\wwwroot\tourimate-production" -ForegroundColor Green
} catch {
...
try {
    ...
    tar -xzf "C:\Users\deploy\tourimate-production.tar.gz" -C "C:\inetpub\wwwroot\tourimate-production" -v
...
$extractedFiles = Get-ChildItem "C:\inetpub\wwwroot\tourimate-production" -Recurse -File
Write-Host "✅ Total files extracted: $($extractedFiles.Count)" -ForegroundColor Green

# Check for key files
$keyFiles = @("tourimate.dll", "tourimate.exe", "appsettings.json", "web.config")
foreach ($file in $keyFiles) {
    if (Test-Path "C:\inetpub\wwwroot\tourimate-production\$file") {
...
try {
    $website = Get-Website -Name "TouriMate API Production" -ErrorAction Stop
